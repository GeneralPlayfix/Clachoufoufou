...
let arrayOfSentences = [
  "Vous allez passer un bon moment avec le",
  "Régalez-vous avec le",
  "Vous allez adorer le",
  "Bonne lecture du",
  "Passez un agréable moment avec le",
  "Vous allez vous régaler avec le",
];
...
function main() {
  ...
  client.on("ready", () => {
    ...
    setTimeout(infoSiteWeb, 0);
    setTimeout(botNormal, 0);
    // setTimeout(test, 150);
  });
}
// client.login("ODU2NTM1MjgyODQ5MDIxOTYz.YNCcpQ.jrFniCVbGRDbcXCd7yPdbuWJNr8");
...
async function discordEmbed() {
  ...
  for (let i = mangas.length - 1; i >= 0; i--) {
    ...
    const embed = new MessageEmbed()
      ...
      .setDescription(`[${rand} ${chap}](${mangas[i].chapLinks})`)
  ...
  // for (sexWord of sexWords) {
  //   fs.appendFile("insults.txt", `${sexWord}\n`, function (err) {
...
function botNormal() {
  ...
  function gotMessage(msg) {
    ...
    for (var i in clachouxRef) {
      if (
        msg.content.toLocaleLowerCase().includes(clachouxRef[i].toLowerCase())
      )
        clachouxRefFoundInText = true;
    }

    if (clachouxRefFoundInText) {
      msg.react(`${emoji}`);
    }
  ...
  if(msg.content == `${prefix.prefix}help`){
    const helpEmbed = new MessageEmbed();
      ...
      for(command of commands){
        ...
        helpEmbed.addFields({ name: prefix.prefix+command.name + nameWithText, value:command.description, inline: false })
      }
      // Set the color of the embed
...
function configFileGenerator(){
  ...
  for(fileCommand of commandFile){
    ...
    if(configFile.indexOf(fileCommand) != -1){
      ...
      fs.appendFile(`${configFolder}${fileCommand}.conf`, 'requireAdminPerms:false\nrequireRoles:false\nrequiredRoles:[]\nwhitelistEnabled:false\nwhitelist:[]', function (err) {
        if (err) throw err;
